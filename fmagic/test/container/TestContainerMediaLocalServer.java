...
public class TestContainerMediaLocalServer extends TestContainer
{
	 ...
	 */
	public TestContainerMediaLocalServer(Context context, TestRunner testRunner,
	 ...
	 *            no test runner is available.
	 */
	public TestContainerMediaLocalServer(TestRunner testRunner)
	{
	...
	public void testMaximumMediaSize()
	{
		...
		try
		{
			 ...
			 */
			Integer lastValueOfMaximumMediaSize = this.getContext().getServerMediaManagerTest().getMaximumMediaSize();
			if (lastValueOfMaximumMediaSize == null) lastValueOfMaximumMediaSize = 0;

			/*
			 *  Check a couple of media resource settings
			 */
			this.getContext().getServerMediaManagerTest().setMaximumMediaSize(this.getContext(), 5000);
			 ...
			 */
			this.getContext().getServerMediaManagerTest().setMaximumMediaSize(this.getContext(), 200);
			this.doMaximumMediaSizeConfiguration(fileList, "Test", "Size3000", 200);

			this.getContext().getServerMediaManagerTest().setMaximumMediaSize(this.getContext(), 700);
			this.doMaximumMediaSizeConfiguration(fileList, "Test", "Size3000", 700);

			this.getContext().getServerMediaManagerTest().setMaximumMediaSize(this.getContext(), 1400);
			this.doMaximumMediaSizeConfiguration(fileList, "Test", "Size3000", 1400);

			/*
			 *  Reset current configuration property
			 */
			this.getContext().getServerMediaManagerTest().setMaximumMediaSize(this.getContext(), lastValueOfMaximumMediaSize);
		}
		catch (Exception e)
		{
			TestManager.servicePrintException(this.getContext(), this, "Unexpected Exception", e);
	...
	private void doUploadFile(String ResourceGroup, String resourceName, String dataIdentifierString, String uploadFileName)
	{
		try
		{
			...
			boolean booleanResult = this.getContext().getServerMediaManagerTest().localStoreMediaFile(this.getContext(), mediaResource, uploadFileName, dataIdentifierString);
	...
	private void doExpiredFiles(String group, String name, CleaningFunctionEnum cleaningFunction)
	{
		try
		{
			...
			{
				daysToKeep = this.getContext().getServerMediaManagerTest().getCleanPendingDaysToKeep();
				dayRange = daysToKeep * 2;
			}
			else if (cleaningFunction == CleaningFunctionEnum.DELETED)
			{
				daysToKeep = this.getContext().getServerMediaManagerTest().getCleanDeletedDaysToKeep();
				dayRange = daysToKeep * 2;
			}
			else
			{
			...
			if (cleaningFunction == CleaningFunctionEnum.PENDING)
			{
				...
				nuOfDeletedFiles = this.getContext().getServerMediaManagerTest().cleanPendingDirectory( this.getContext(), mediaResource, daysToKeep);
			}
			else if (cleaningFunction == CleaningFunctionEnum.DELETED)
			{
				remainingFilePath = mediaResource.mediaFileGetDeletedFilePath(this.getContext());
				nuOfDeletedFiles = this.getContext().getServerMediaManagerTest().cleanDeletedDirectory(this.getContext(), mediaResource, daysToKeep);
			}
			else
			{
				TestManager.servicePrintError(this.getContext(), this, "CleaningFunctionEnum not supported", null);
	...
	private void doExpiredObsoleteFiles(String group, String name)
	{
		try
		{
			...
			int daysToKeep = this.getContext().getServerMediaManagerTest().getCleanObsoleteDaysToKeep();
			...
			int nuOfDeletedFiles = this.getContext().getServerMediaManagerTest().cleanRegularDirectory(this.getContext(), mediaResource, daysToKeep);
			TestManager.assertGreaterThan(this.getContext(), this, null, nuOfDeletedFiles, 0);
			TestManager.assertLowerThan(this.getContext(), this, null, nuOfDeletedFiles, nuOfFilesCreated);
	...
	private void doCleaningAll()
	{
		...
		try
		{
			...
			int currentCleanDeletedDaysToKeep = this.getContext().getServerMediaManagerTest().getCleanDeletedDaysToKeep();
			this.getContext().getServerMediaManagerTest().setCleanDeletedDaysToKeep(this.getContext(), daysToKeep * 3);

			// Invoke regular clean function of the media manager
			int nuOfDeletedFiles = this.getContext().getServerMediaManagerTest().cleanAll(this.getContext());
			...
			this.getContext().getServerMediaManagerTest().setCleanDeletedDaysToKeep(this.getContext(), currentCleanDeletedDaysToKeep);
			...
			if (settingOfAttributeMediaSize != null)
			{
				for (int i = 0; i < fileList.size(); i++)
				{
					...
					if (currentMediaFileSize > maximumMediaFileSize)
					{
						...
						boolean booleanResult = this.getContext().getServerMediaManagerTest().localStoreMediaFile(this.getContext(), media, fileList.get(i), name);
