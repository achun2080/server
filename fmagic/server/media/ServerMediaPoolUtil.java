...
public class ServerMediaPoolUtil
{
	...
	private boolean addCommandToMainQueue(Context context, String mediaResourceIdentifier, String mediaPendingFileName, String dataIdentifier)
	{
		...
		try
		{
			...
			// Logging
			context.getNotificationManager().notifyLogMessage(context, NotificationManager.SystemLogLevelEnum.NOTICE, "Media pool command added to MAIN queue");
	...
	private boolean addCommandToSecondaryQueue(Context context, String mediaResourceIdentifier, String mediaPendingFileName, String dataIdentifier)
	{
		...
		try
		{
			...
			// Logging
			context.getNotificationManager().notifyLogMessage(context, NotificationManager.SystemLogLevelEnum.NOTICE, "Media pool command added to SECONDARY queue");
			context.getNotificationManager().notifyLogMessage(context, NotificationManager.SystemLogLevelEnum.CODE, command.toString());
	...
	public boolean poolUploadMediaFileToPool(Context context, ResourceContainerMedia mediaResourceContainer, String uploadFileNamePath, String dataIdentifier)
	{
		...
		if (this.mediaManager.getMediaUtil().isMaximumFileSizeExceeded(context, uploadFileNamePath) == true)
		{
			String errorString = "--> UPLOAD ON POOL: Maximum allowed media file size exceeded.";
			errorString += "\n--> Media resource identifier: '" + mediaResourceContainer.getRecourceIdentifier() + "'";
			errorString += "\n--> File name of the file to be uploaded: '" + uploadFileNamePath + "'";

			Integer maximumFileSize = this.mediaManager.getMaximumMediaSize();
	 ...
	 */
	public boolean queueExecuteCommand(Context context, ServerMediaPoolCommand serverMediaPoolCommand, boolean processMainServerOnly)
	{
		/*
		 * Check variables and conditions
		 */
		...
		try
		{
			...
			if (numberOfserverInPool > 0)
			{
				for (ConnectionContainer connectionContainer : this.mediaPoolList.values())
				{
					...
					if (context.getMediaManager().commandCheckOnServer(context, connectionContainer, mediaResourceContainer, fileType, dataIdentifier, hashValue) == true) return true;

					// Upload file to media pool
					if (context.getMediaManager().commandUploadToServer(context, connectionContainer, mediaResourceContainer, uploadPendingFileNamePath, dataIdentifier) == false)
					{
